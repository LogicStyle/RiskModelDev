...
tmp.subf <- NULL  # substract factors as you need
...
alphaf_IC2 <- rownames(ngrouptable[ngrouptable$Annualized.Return>=0.05 & ngrouptable$Annualized.Sharpe.Rf.0..>=0.3 & ngrouptable$HitRatio>0.55,])
tmp.addf <- NULL  # add factors as you need
...
MC.chart.fCorr(TSF_ICalpha,'4 years')
```
...
```{r factorselect}
TSFR <- na.omit(TSFR)
reg_results <- reg.factor.select(TSFR)
```  
